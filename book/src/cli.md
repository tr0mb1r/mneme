...
| `mneme daemon` | **v1.1 default MCP server entry point** per ADR-0012. Binds `~/.mneme/run/mneme.sock` (Unix domain socket on macOS / Linux; Windows named pipe lands in M4), accepts multiple MCP clients concurrently, gates each connection on the auth handshake (`MNEME-AUTH: <token>\n` per ADR-0012 D3), serves them via `tokio::spawn` per connection. Acquires `~/.mneme/.lock` for the lifetime of the process. Storage writes serialise through the single-writer seam (D8). Auto-shuts-down after `[daemon].idle_timeout_minutes` (default 30; `0` disables) of no clients. SIGTERM drains in-flight clients up to 30 s before runtime tears them down. Emits `session_start` on boot and `session_end` on graceful exit (ADR-0009). |
