...
## [1.0.0] — main (release candidate, tag target 2026-05-18)

> Heading: this section marks the v1.0 cut. Per the planning doc the
> v1.0 soak clock started 2026-04-29; this tag promotes the cumulative
> stability of the v0.2.x series (MCP wire surface, memory layers,
> orchestrator, redb storage, HNSW semantic index, daemon-mode
> precursor in `mneme run`) to a 1.x API contract. All v0.2.6 features
> ship under the new version number without behavioral change.
...
## [1.1.1] — develop (release candidate, tag target 2026-05-30)

> Heading: this section accumulates what will land as `v1.1.1` once
> the v1.0 soak completes (clock started 2026-04-29; reached day 17
> on 2026-05-16) and `v1.0` is tagged first. The crate version on
> `develop` is `1.1.1` so dogfood binaries report the version they're
> being tested against; the tagged release will follow the planning
> doc's `v1.0 → v1.1` order. Initially titled `[1.1.0] — preview`;
> renamed to `[1.1.1]` on 2026-05-16 after the D12 client wiring +
> drain broadcast + refactor batch landed on top of the original
> v1.1.0 scope.
