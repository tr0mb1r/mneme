...
> Heading: this section covers `v1.1.1`, tagged 2026-05-23 after the
> v1.0 soak. Initially titled `[1.1.0] — preview`; renamed to
> `[1.1.1]` on 2026-05-16 after the D12 client wiring + drain
> broadcast + refactor batch landed on top of the original v1.1.0
> scope.

### Added (post-draft v1.1.1 work, 2026-05-16)

- **D12 auto-spawn + MCP auto-reconnect on daemon restart**
  (commits `742003e`, wait-budget bumped at `8846268`, ADR-0012
  §D12). `mneme client` transparently spawns the daemon if it's
  not running (exponential backoff 10 ms → 30 s budget) and
  detects mid-session socket EOF to reconnect with replay of the
  MCP initialize handshake (prevents `-32002` errors). Background
  stdin reader buffers inbound frames via mpsc channel during
  the reconnect window. Integration tests:
  `daemon_is_spawned_by_detach_primitive`,
  `client_reconnects_after_daemon_restart`,
  `client_auto_spawns_daemon_on_first_connect` in
  `tests/daemon_e2e.rs`. Removes the manual "remember to run
  `mneme daemon` first" step from every agent's setup.
- **Cursor + OpenCode per-agent installers** (commit `1afe24e`).
  `mneme init cursor` writes `~/.cursor/mcp.json` (uniform path
  across macOS/Linux/Windows) with the `mneme client` MCP entry;
  `mneme init opencode` writes `~/.config/opencode/opencode.json`
  with `mcp.mneme` plus a mneme-owned instruction file referenced
  via OpenCode's `instructions[]` array (auto-loads into the system
  prompt on every session — zero paste step). Both follow the
  Claude Code reference installer's idempotency + atomic-write +
  `--show`/`--upgrade`/`--uninstall` contract.
- **Active-drain broadcast on daemon shutdown** (commit `1664a8b`).
  Pre-fix: per-conn spawned tasks blocked in `Server::run`'s
  `read_frame.await` with no cancellation hook → `wait_for_drain`
  always timed out the full `DRAIN_DEADLINE` (30 s) when clients
  were connected; total daemon exit ~40 s; `mneme stop`'s 10 s
  timeout needed 3–4 retries. Fix: `tokio::sync::watch<bool>`
  shutdown channel; per-conn tasks `select!` `Server::run`
  against `shutdown_rx.changed()`; on SIGTERM the daemon-level
  `select!` resolves, `shutdown_tx.send(true)` wakes connected
  tasks, they drop socket halves, clients see EOF, drain
  completes in ms. `mneme stop` `SHUTDOWN_TIMEOUT` bumped 10 s →
  45 s as safety. Regression test
  `daemon_drains_idle_clients_promptly_on_sigterm` (strengthened
  at `6dcbad5` with log invariants).
- **Codebase refactor batch** (2026-05-16). Coordinated cross-
  agent batch to reduce technical-debt surface:
  - `0810116` — `MEM_KEY_PREFIX` const consolidated from 9
    duplicate definitions into one `pub(crate)` in
    `src/storage/mod.rs`. Snapshot scheduler extracted from
    `src/memory/semantic.rs` (1517 → 1385 lines) into a new
    `src/memory/snapshot_scheduler.rs` submodule.
  - `2ebf91f` — failure-mode + unit test coverage for MCP
    resources (`context.rs`, `procedural.rs`, `recent.rs`) +
    `stats` aggregator + previously-untested tools
    (`summarize_session`, `recall_recent`, `unpin`). Promoted
    from P3 to high priority because the MCP resources are read
    by every agent on turn 1.
  - `318772d` — `tools/call` triage (99 lines: success /
    soft-error / hard-error dispatch + L3 emission) extracted
    from `src/mcp/server.rs` (1273 → 1003 lines) into
    `src/mcp/dispatch.rs`.
  - `6dcbad5` — `ConnectionGuard` RAII for the daemon's
    `active_clients` counter (replaces three manual `fetch_sub` +
    `tracing::info!` sites). Includes a lifetime fix on top of
    the initial OpenCode draft + a strengthened regression test
    with stderr-log invariants to catch a class of lifetime bug
    going forward.
- **Supported-agents documentation page** in mdBook
  (`book/src/agents.md`). Status table for Tier-1 agents (Claude
  Code, Claude Desktop, Cursor, OpenCode) with per-agent installer
  / config-file / hooks-API / auto-spawn / auto-reconnect columns,
  plus a Tier-2 deferred table (cline, codex, gemini-cli) and a
  known-limitations section flagging the multi-agent scope-leak
  caveat.
